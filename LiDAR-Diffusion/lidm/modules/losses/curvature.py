...
class Curvature(nn.Module):
    def __init__(self, dataset_config=dict()):
        ...
        # 计算曲率的卷积核
        self.conv_kernel = torch.tensor([1, 1, 1, 1, 1, -10, 1, 1, 1, 1, 1], dtype=torch.float32).view(1, 1, 1, -1).to(self.device)
    ...
    def register_conversion(self):
        ...
        self.register_buffer('cos_yaw', torch.cos(to_torch(yaw)).to(self.device))
        self.register_buffer('sin_yaw', torch.sin(to_torch(yaw)).to(self.device))
        self.register_buffer('cos_pitch', torch.cos(to_torch(pitch)).to(self.device))
        self.register_buffer('sin_pitch', torch.sin(to_torch(pitch)).to(self.device))
    ...
    def curvature(self, input_xyz):
        input_xyz = input_xyz.to(self.device)
        ...
        diff = torch.cat([diff, torch.zeros((B, H, 1), device=input_xyz.device)], dim=-1)

        mask1 = diff > 0.1
        depth_ratio = depth[:, :, 1:] / depth[:, :, :-1]
        diff_ratio = torch.sqrt(((input_xyz[:, :, :, 1:] - input_xyz[:, :, :, :-1] * depth_ratio.unsqueeze(1)) ** 2).sum(dim=1))
        diff_ratio = torch.cat([torch.ones((B, H, 1), device=input_xyz.device), diff_ratio], dim=-1)
        ...
        mask_outlier = (diff > 0.001 * dis) & (diff2 > 0.001 * dis)
